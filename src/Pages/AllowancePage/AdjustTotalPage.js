...
export default function AdjustTotalPage() {
  ...
  return (
    <div className="d-flex justify-content-center w-100 flex-column align-items-center" style={{ height: "70vh" }}>
      <h4 className="title">{activeFamilyMember ? `New total for ${activeFamilyMember}'s allowance?` : null}</h4>

      <div className="w-75">
        <FormInput titles={["Amount"]} placeholder={currentAllowance[activeFamilyMember].currentTotal} handleSubmit={handleSubmit} />
      </div>
    </div>
  )
}
